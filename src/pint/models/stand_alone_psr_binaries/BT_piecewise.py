...
class BTpiecewise(BTmodel):
    ...
    def piecewise_parameter_loader(self, valDict=None):
        ...
        if (
            ...
            for (
                ...
                valDict.items()
            ):  # Searches through updates for keys prefixes matching T0X/A1X, can be allowed to be more flexible with param+"X_" provided param is defined earlier. Arbitrary piecewise parameter model

                if key[0:4] == "T0X_" or key[0:4] == "A1X_":
                    piece_index.append((key[4:8]))  # appends index to array
            piece_index = np.unique(
                piece_index
            ...
            )  # sorts the array chronologically by lower edge of each group,correctly works for unordered pieces (i.e. index 0 can correspond to an arbitrary group of data at any time)
                ...
                if hasattr(self, "_t") is True:
                    if (self._t) is not None:
                        ...
                        (
                            self.T0X_per_toa,
                            self.A1X_per_toa,
                        ) = self.piecewise_parameter_from_information_array(
                            self._t
        ...
        for i in self.group_index_array:
            if i != -1:
                for j in range(len(self.piecewise_parameter_information)):
                    if (
                        self.piecewise_parameter_information[j][0] == i
                    ):  # searches the 5 x n array to find the index matching the toa_index
                        T0X_per_toa.append(
                            self.piecewise_parameter_information[j][1].value
                        )
                        A1X_per_toa.append(
                            self.piecewise_parameter_information[j][2].value
                        )
    ...
    def get_group_boundaries(self):
        ...
        if hasattr(self, "piecewise_parameter_information"):
            for i in range(0, len(self.piecewise_parameter_information)):
                lower_group_edge.append(
                    self.piecewise_parameter_information[i][3].value
                )
                upper_group_edge.append(
                    self.piecewise_parameter_information[i][4].value
                )
    ...
    def get_tt0(self, barycentricTOA):
        ...
        if len(barycentricTOA) > 1:
            if len(self.piecewise_parameter_information) > 0:
                self.toa_belongs_in_group(
                    barycentricTOA
                )  # Defines object's index for each toa as an array of length = len(self._t)
                (
                    self.T0X_per_toa,
                    self.A1X_per_toa,
                ) = self.piecewise_parameter_from_information_array(
    ...
    def d_delayL1_d_par(self, par):
        ...
        if (
            par_temp is None
        ):  # to circumvent the need for list of d_pars = [T0X_0,...,T0X_i] use par_temp
