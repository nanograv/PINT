...
class SolarSystemShapiro(DelayComponent):
    ...
    def solar_system_shapiro_delay(self, toas, acc_delay=None):
        ...
        tbl_mask = tbl if np.all(non_bary_mask) else tbl[non_bary_mask]

        psr_dir = self._parent.ssb_to_psb_xyz_ICRS(epoch=tbl_mask["tdbld"])
        delay[non_bary_mask] += self.ss_obj_shapiro_delay(
            tbl_mask["obs_sun_pos"], psr_dir, self._ss_mass_sec["sun"]
        )
        try:
            if self.PLANET_SHAPIRO.value:
                for pl in ("jupiter", "saturn", "venus", "uranus", "neptune"):
                    delay[non_bary_mask] += self.ss_obj_shapiro_delay(
                        tbl_mask[f"obs_{pl}_pos"],
                        psr_dir,
