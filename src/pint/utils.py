...
def ECL_to_ICRS(model):
    """Convert model from PulsarEcliptic to ICRS.

    Parameters
    ----------
    model : pint.models.timing_model.TimingModel
        PINT timing model in PulsarEcliptic frame
    ...
    pint.models.timing_model.TimingModel
        In ICRS frame
    """
    m_eq = astrometry.AstrometryEquatorial()

    # transfer over parallax and POSEPOCH: don't need to change
    m_eq.PX = model.PX
    m_eq.POSEPOCH = model.POSEPOCH
    # get RA, DEC, PM
    p = model.get_params_as_ICRS()
    m_eq.RAJ.quantity = p["RAJ"]
    m_eq.DECJ.quantity = p["DECJ"]
    m_eq.PMRA.quantity = p["PMRA"]
    m_eq.PMDEC.quantity = p["PMDEC"]
    ...
    c = SkyCoord(
        ...
        obliquity=OBL[model.ECL.value],
        obstime=model.POSEPOCH.quantity,
        pm_lon_coslat=model.ELONG.uncertainty / dt,
        pm_lat=model.ELAT.uncertainty / dt,
    ...
    c = SkyCoord(
        ...
        obliquity=OBL[model.ECL.value],
        obstime=model.POSEPOCH.quantity,
        pm_lon_coslat=model.PMELONG.uncertainty,
        pm_lat=model.PMELAT.uncertainty,
        frame=PulsarEcliptic,
    )
    c_ICRS = c.transform_to(coords.ICRS)
    ...
    m_eq.RAJ.frozen = model.ELONG.frozen
    m_eq.DECJ.frozen = model.ELAT.frozen
    m_eq.PMRA.frozen = model.PMELONG.frozen
    m_eq.PMDEC.frozen = model.PMELAT.frozen

    m_copy = copy.deepcopy(model)
    m_copy.remove_component("AstrometryEcliptic")
    m_copy.add_component(m_eq)
    return m_copy


def ICRS_to_ECL(model, ecl="IERS2010"):
    """Convert model from ICRS to PulsarEcliptic.

    Parameters
    ----------
    model : pint.models.timing_model.TimingModel
        PINT timing model in ICRS frame
    ecl : str, optional
    ...
    pint.models.timing_model.TimingModel
        In ICRS frame
    """
    m_ecl = astrometry.AstrometryEcliptic()

    # transfer over parallax and POSEPOCH: don't need to change
    m_ecl.PX = model.PX
    m_ecl.POSEPOCH = model.POSEPOCH
    # get ELONG, ELAT, PM
    c = model.coords_as_ECL(ecl=ecl)
    ...
    dt = 1 * u.yr
    c = SkyCoord(
        ra=model.RAJ.quantity,
        dec=model.DECJ.quantity,
        obstime=model.POSEPOCH.quantity,
        pm_ra_cosdec=model.DECJ.uncertainty / dt,
        pm_dec=model.DECJ.uncertainty / dt,
        frame=coords.ICRS,
    )
    ...
    c = SkyCoord(
        ...
        dec=model.DECJ.quantity,
        obstime=model.POSEPOCH.quantity,
        pm_ra_cosdec=model.PMRA.uncertainty,
        pm_dec=model.PMDEC.uncertainty,
        frame=coords.ICRS,
    )
    c_ECL = c.transform_to(PulsarEcliptic(ecl=ecl))
    ...
    # freeze comparable parameters
    m_ecl.ELONG.frozen = model.RAJ.frozen
    m_ecl.ELAT.frozen = model.DECJ.frozen
    m_ecl.PMELONG.frozen = model.PMRA.frozen
    m_ecl.PMELAT.frozen = model.PMDEC.frozen

    m_copy = copy.deepcopy(model)
    m_copy.remove_component("AstrometryEquatorial")
    m_copy.add_component(m_ecl)
    return m_copy
