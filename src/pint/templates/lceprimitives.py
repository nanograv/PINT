...
class LCEPrimitive(LCPrimitive):
    ...
    def get_bounds(self,free=True):
        if free:
            return np.append(
                    np.asarray(self.bounds)[self.free],np.asarray(self.slope_bounds)[self.slope_free],axis=0)
        return np.append(
                np.asarray(self.bounds),np.asarray(self.slope_bounds),axis=0)
...
class LCEWrappedFunction(LCEPrimitive,LCWrappedFunction):
    ...
    def hessian(self,phases,log10_ens=3,free=False):
        ...
        """
        t = self._make_p(log10_ens); e = t[0]
