...
class emcee_fitter(Fitter):
    ...
    def lnprior(self, theta):
        ...
        """
        lnsum = 0.0
        for val, mn, sig, key in zip(theta, self.fitvals, self.fiterrs, self.fitkeys):
            ...
            if key == "GLEP_1":
                ...
                lnsum += -np.log(sig * np.sqrt(2.0 * np.pi)) - (val - mn) ** 2.0 / (
                    2.0 * sig ** 2.0
                )
        return lnsum
    ...
    def lnposterior(self, theta):
        ...
        if "PX" in self.fitkeys and self.model.PX.value < 0.0:
            return -np.inf
        if "SINI" in self.fitkeys and (
            self.model.SINI.value > 1.0 or self.model.SINI.value < 0.0
        ):
            return -np.inf
        # Do we really need to check both E and ECC or can the model param alias handle that?
        if "E" in self.fitkeys and (
            self.model.E.value < 0.0 or self.model.E.value >= 1.0
        ):
            return -np.inf
        if "ECC" in self.fitkeys and (
            self.model.ECC.value < 0.0 or self.model.ECC.value >= 1.0
        ):
            return -np.inf
