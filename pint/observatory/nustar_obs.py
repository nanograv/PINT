...
def load_orbit(orb_filename):
    ...
    log.info('Building orb table covering MJDs {0} to {1}'.format(mjds_TT.min(), mjds_TT.max()))
    orb_table = Table([mjds_TT, X, Y, Z, Vx, Vy, Vz],
...
class NuSTARObs(SpecialLocation):
    ...
    def get_gcrs(self, t, ephem=None, grp=None):
        ...
        in GCRS coordinates.
        '''
        return np.array([self.X(t.tt.mjd),
                         self.Y(t.tt.mjd),
    ...
    def posvel(self, t, ephem):
        ...
        nustar_pos_geo = np.array([self.X(t.tt.mjd),
                                   self.Y(t.tt.mjd),
                                   self.Z(t.tt.mjd)])*self.FPorb['X'].unit
        nustar_vel_geo = np.array([self.Vx(t.tt.mjd),
                                   self.Vy(t.tt.mjd),
                                   self.Vz(t.tt.mjd)])*self.FPorb['Vx'].unit
        nustar_posvel = PosVel(nustar_pos_geo, nustar_vel_geo, origin='earth',
                               obj='nustar')
