...
class Glitch(TimingModel):
    ...
    def setup(self):
        ...
        for glphnm in glphparams:
            ...
            # Check if the other glicth F0 and F1 exists, if not add as zero parameter
            for glf in ["GLF0_", "GLF1_"]:
                if not hasattr(self,glf+'%d'%idx):
                    glf0exp = getattr(self,glf+'1') # The first glf0 and glf1 should be there
        ...
        for glf0d in glf0dparams:
            ...
            idx = df0d.index
            if not hasattr(self,'GLPH_%d'%idx): # Check if the decay belongs to a glitch
                msg = 'Glitch frequency decay index has to match one glicth phase'
                msg += ' index'
                raise MissingParameter("Glitch", 'GLPH_%d'%idx, msg)

            if not hasattr(self,'GLTD_%d'%idx):
                if df0d.value != 0.0:
                    msg = 'None zero GLTD_%d parameter needs a GLTD_%d parameter'%(idx,idx)
