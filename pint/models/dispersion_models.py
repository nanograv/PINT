...
class Dispersion(TimingModel):
    """This class provides a base for dispersion measure model.
    """
...
class DispersionBasic(Dispersion):
    """This is a simple dispersion model with a constant dispersion measure.
    """
    def __init__(self):
        super(DispersionBasic, self).__init__()
        self.dispersion_model_name = 'basic'
        self.delay_funcs['L1'] += [self.basic_dispersion_delay]
    def setup(self):
        super(DispersionBasic, self).setup()


    def basic_dispersion_delay(self, toas):
        """Return the simple dispersion delay at each toa."""
        try:
            bfreq = self.barycentric_radio_freq(toas)
        except AttributeError:
            warn("Using topocentric frequency for dedispersion!")
...
class DispersionDMX(Dispersion):
    """This class provides a timing model for a non-constant dispersion measure
    model. Dispersion varies as time.
    Added variables:
    DMX, DMX_, DMXR1_, DMXR2_
    """
    ...
    def __init__(self):
        ...
        self.dispersion_model_name = 'dmx'
        self.delay_funcs['L1'] += [self.DMX_dispersion_delay]
    ...
    def DMX_dispersion_delay(self, toas):
        """Return the dispersion delay at each toa for DMX model."""
        try:
            bfreq = self.barycentric_radio_freq(toas)
        except AttributeError:
            warn("Using topocentric frequency for dedispersion!")
