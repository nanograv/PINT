    ...
    for b_name in base:
        ...
        #for p,v in zip(['E',], [per_step['E'],]):
            ...
            if pn != '':
                ...
                try:
                    T2_pval, T2_unc, T2_post_rms, T2_chi = check_tempo2_output(tempo_par, tim, p, result_par)
                except:
                    T2_pval, T2_unc, T2_post_rms, T2_chi = check_tempo2_output(tempo_par, tim, pn, result_par)
                # Tempo_fitting
                T_m, T_pre_rms, T_post_rms, T_chi = check_tempo_output(tempo_par, tim, result_par)
                ...
                pt_row.append((pn, p_fit_value, p_tempo, diff_PINT_T, rela_diff_T,
                               p_fit_unc, p_tempo_unc, ucdiff_T ,P_red_chi, T_chi*f.toas.ntoas,))

                pt2_row.append((pn, p_fit_value, T2_pval, diff_PINT_T2, rela_diff_T2,
                                p_fit_unc, T2_unc, ucdiff_T2, P_red_chi, T2_chi*f.toas.ntoas,))
                print 'Tempo row',(pn, p_fit_value, p_tempo, diff_PINT_T, rela_diff_T,
                               p_fit_unc, p_tempo_unc, ucdiff_T ,P_red_chi, T_chi*f.toas.ntoas,)
                print 'Tempo2 row', (pn, p_fit_value, T2_pval, diff_PINT_T2, rela_diff_T2,
                                p_fit_unc, T2_unc, ucdiff_T2, P_red_chi, T2_chi*f.toas.ntoas,)

        if pt_row != []:
            pt_table = Table(rows=pt_row, names=('Parameter', 'PINT_postFit_value', 'T_value', \
                                            'par_diff(PINT-T)', \
                                            'relative_diff_(PINT-T)/Tempo_uncertainty', \
                                            'PINT_postFit_uncertainty', \
                                            'Tempo_uncertainty', \
                                            'PINT_uncertainty/tempo_uncertainty', 'PINT_chi', 'T_chi',),
                   meta={'name': 'PINT_Tempo result table'}, dtype=('S10', 'f8', 'f8', 'f8', 'f8', \
                                                                    'f8','f8', 'f8','f8', 'f8',))
        if pt2_row != []:
            pt2_table = Table(rows=pt2_row, names=('Parameter', 'PINT_postFit_value', 'T2_value',
                                            'par_diff(PINT-T2)',
                                            'relative_diff_(PINT-T2)/Tempo2_uncertainty',
                                            'PINT_postFit_uncertainty',
                                            'Tempo2_uncertainty',
                                            'PINT_uncertainty/tempo2_uncertainty', 'PINT_chi', 'T2_chi'),
                   meta={'name': 'PINT_Tempo2 result table'}, dtype=('S10', 'f8', 'f8', 'f8', 'f8', 'f8','f8', 'f8','f8', 'f8'))
