...
def phaseogram(mjds, phases, weights=None, title=None, bins=100, rotate=0.0, size=5, alpha=0.25, file=False):
    """
    ...
    if weights is None:
        ...
        for ph,ww in zip(phases,weights):
            bin = int(ph*bins)
            profile[bin] += ww
        ax1.step(np.arange(2*bins)/bins,np.concatenate((profile,profile)),lw=1.5,color='k')
        ax1.set_ylim([0.0,1.1*profile.max()])
