...
def make_toas_to_go_with_two_piece_model(build_piecewise_model_with_two_pieces):
    ...
    toas = pint.simulation.make_fake_toas_uniform(
        m_piecewise.START.value + 1, m_piecewise.FINISH.value - 1, 20, m_piecewise
...
def build_piecewise_model_with_two_pieces(build_model):
    ...
    for i in range(0, 2):
        piecewise_model.add_group_range(
            piecewise_model.START.value + spacing * i,
            piecewise_model.START.value + spacing * (i + 1),
            j=i,
        )
        piecewise_model.add_piecewise_param(
            "A1", "ls", piecewise_model.A1.value + i * 1e-5, i
        )
        piecewise_model.add_piecewise_param(
            "T0", "d", piecewise_model.T0.value + i * 1e-5, i
        )
...
def test_add_piecewise_parameter(build_model):
    ...
    for i in range(0, n):
        m_piecewise.add_group_range(
            m_piecewise.START.value + 501 * i, m_piecewise.START.value + 1000 * i, j=i
        )
        ...
        for copy_key, copy_value in param_dict.items():
            if key == copy_key:  # search both pars for identical keys
                if value.quantity == copy_value.quantity:
                    comparison = (
                        comparison + 1
                    )  # iterates up to all matched keys, which should be all, hence total number of keys
...
def test_paramX_per_toa_matches_corresponding_model_value(
    ...
    do_arrays_match = np.array_equal(
...
def add_groups_and_make_toas(build_model, build_piecewise_model_with_two_pieces, param):
    ...
    if param == "non-overlapping complete group coverage":
        ...
        model2.add_group_range(
            model2.START.value - 1, model2.START.value + spacing + 100, j=0
        )
        model2.add_group_range(
            model2.START.value + spacing - 100, model2.FINISH.value + 1, j=1
        )
        ...
        model3.add_group_range(
            model3.START.value + spacing, model3.FINISH.value + 1, j=0
        )
...
def make_generic_toas(model):
    ...
    return pint.simulation.make_fake_toas_uniform(
        model.START.value, model.FINISH.value - 1, 20, model
    )
...
def check_if_in_piece(model, toas):
    ...
    for i in range(model.get_number_of_groups()):
        ...
        is_toa_in_each_group.append(
            model.does_toa_reference_piecewise_parameter(toas, par)
        )
...
def return_truth_array_based_on_group_boundaries(model, barycentric_toa):
    ...
    truth_array_comparison = [
        [barycentric_toa.value <= upper_edge_of_lower_group],
        [barycentric_toa.value >= lower_edge_of_upper_group],
    ]
...
@pytest.mark.parametrize(
    "param",
    [
        "non-overlapping complete group coverage",
        "overlapping groups",
        "non-complete group coverage",
    ],
)
def test_does_toa_lie_in_group(
    build_model, build_piecewise_model_with_two_pieces, param
):
    m_piecewise, toas = add_groups_and_make_toas(
        build_model, build_piecewise_model_with_two_pieces, param
    )
    ...
    if param == "non-overlapping complete group coverage":
        ...
        does_in_piece_and_mjd_method_agree = np.array_equal(
            are_toas_within_group_boundaries_mjd_method_per_parameter,
            is_toa_in_each_group,
        )
        ...
        where_toas_should_be_in_group_1 = np.where(
            are_toas_within_group_boundaries_mjd_method_per_parameter[0][0]
            != are_toas_within_group_boundaries_mjd_method_per_parameter[1][0],
            are_toas_within_group_boundaries_mjd_method_per_parameter[0][0],
            are_toas_within_group_boundaries_mjd_method_per_parameter[0][0]
        ...
        where_toas_should_be_in_group_2 = np.where(
            are_toas_within_group_boundaries_mjd_method_per_parameter[0][0]
            != are_toas_within_group_boundaries_mjd_method_per_parameter[1][0],
            are_toas_within_group_boundaries_mjd_method_per_parameter[1][0],
            are_toas_within_group_boundaries_mjd_method_per_parameter[0][0]
        ...
        are_toas_within_group_boundaries_mjd_method_per_parameter = [
            where_toas_should_be_in_group_1,
            where_toas_should_be_in_group_2,
        ]
...
def test_residuals_in_groups_respond_to_changes_in_corresponding_piecewise_parameter(
    build_model, build_piecewise_model_with_two_pieces, param, index
):
    m_piecewise, toas = add_groups_and_make_toas(
        build_model, build_piecewise_model_with_two_pieces, "overlapping groups"
    )
    # m_piecewise.setup()
    ...
    are_correct_residuals_responding = np.array_equal(
        have_residuals_changed, should_residuals_change
    )
...
@pytest.mark.parametrize(
    "param, index",
    [("T0X", 0), ("T0X", 1), ("T0X", -1), ("A1X", 0), ("A1X", 1), ("A1X", -1)],
)
...
def test_d_delay_in_groups_respond_to_changes_in_corresponding_piecewise_parameter(
    ...
    if index == -1:
        is_d_delay_changing = np.invert(
            np.isclose(
                m_piecewise_temp.d_binary_delay_d_xxxx(toas, param[0:2], None).value,
                0,
                atol=1e-11,
                rtol=0,
            )
        ...
        is_d_delay_changing = np.invert(
            np.isclose(
                m_piecewise_temp.d_binary_delay_d_xxxx(
                    toas, parameter_string, None
                ).value,
                0,
                atol=1e-11,
                rtol=0,
    ...
    both_arrays_should_be_equal = np.array_equal(
        is_d_delay_changing, should_d_delay_be_changing
    )
...
def add_relative_offset_for_derivatives(index, param, model, offset_size, plus=True):
    ...
    if plus is True:
        if hasattr(m_piecewise_temp, parameter_string):
            ...
            parameter_string = param[0:2]
            getattr(m_piecewise_temp, parameter_string).value = (
                getattr(m_piecewise_temp, parameter_string).value + offset_size
            )
            m_piecewise_temp.setup()
        return m_piecewise_temp
    else:
...
def get_d_delay_d_xxxx(toas, model, parameter_string):
    ...
    d_delay_temp = rs_temp.model.d_binary_delay_d_xxxx(
        toas, parameter_string, acc_delay=None
    )
    return d_delay_temp
