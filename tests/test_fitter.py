...
plt.close()
p1=plt.errorbar(xt,f.resids.time_resids,yerr.value,fmt='bo');

# Do a 4-parameter fit
f.set_fitparams('F0','F1','RA','DEC')
f.call_minimize()
print('chi^2 is %0.2f after 4-param fit' % f.resids.chi2)
p2=plt.errorbar(xt,f.resids.time_resids,yerr.value,fmt='go');
...
p3=plt.errorbar(xt,f.resids.time_resids,yerr.value,fmt='ms');

f.set_fitparams('F1')
f.call_minimize()
print('chi^2 is %0.2f after fitting just F1 with default method="Powell"' % f.resids.chi2)
p4=plt.errorbar(xt,f.resids.time_resids,yerr.value,fmt='k^');
...
p5=plt.errorbar(xt,f.resids.time_resids,yerr.value,fmt='rs');
...
# global minimum.
f.call_minimize()
p6=plt.errorbar(xt,f.resids.time_resids,yerr.value,fmt='g^');
print(f.fitresult)
