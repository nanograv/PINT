...
p3=plt.errorbar(xt,f.resids.resids,yerr,fmt='ms');

f.set_fitparams('F1')
f.call_minimize()
print('chi^2 is %0.2f after fitting just F1 with default method="Powell"' % f.resids.chi2)
p4=plt.errorbar(xt,f.resids.resids,yerr,fmt='k^');
...
p5=plt.errorbar(xt,f.resids.resids,yerr,fmt='rs');
...
# global minimum.
f.call_minimize()
p6=plt.errorbar(xt,f.resids.resids,yerr,fmt='g^');
print(f.fitresult)
