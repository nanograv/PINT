    ...
    def test_paramchange(self):
        ...
        for verbosity in verbosities:
            for pn in model.params_ordered:
                if pn.startswith('DMX') or pn in ['PSR','START','FINISH'] or 'EPOCH' in pn:
                    continue
                param=getattr(model,pn)
                param_cp=getattr(modelcp,pn)
                if type(param_cp.quantity) in [str, astropy.time.core.Time] or param_cp.quantity is None:
                    continue
                if param_cp.uncertainty!=None:
                    param_cp.quantity=param_cp.quantity+factor*param_cp.uncertainty
                else:
                    if type(param)==pint.models.parameter.boolParameter:
                        ...
                        param_cp.value+=3.
                model.compare(modelcp,threshold_sigma=threshold_sigma,verbosity=verbosity)
            ...
            for pn in model.params_ordered:
                ...
                if param_cp.uncertainty!=None:
                    ...
                    if type(param)==pint.models.parameter.boolParameter:
                        ...
                        param_cp.uncertainty=3.*param_cp.units
