...
def test_pintify_parfile_from_string():
    m = ModelBuilder()
    pint_dict, unknow = m._pintify_parfile(StringIO(base_par))
...
def test_pintify_parfile_from_parfile_dict():
    m = ModelBuilder()
    par_dict = parse_parfile(StringIO(base_par))
    pint_dict, unknow = m._pintify_parfile(par_dict)
    assert len(pint_dict) == len((base_par.strip()).split('\n'))
    assert len(unknow) == 0
...
def test_pintify_parfile_repeat_line_with_repeat_param():
    ...
    repeat_par += "\nECORR -fe 1 1 2\nECORR -fe 2 1 2"
    m = ModelBuilder()
    pint_dict, unknow = m._pintify_parfile(StringIO(repeat_par))
...
def test_parse_parfile_index_param():
    ...
    m = ModelBuilder()
    pint_dict, unknow = m._pintify_parfile(StringIO(indexed_par))
...
def test_pintify_parfile_alises():
    ...
    m = ModelBuilder()
    pint_dict, unknow = m._pintify_parfile(StringIO(aliases_par))
...
def test_pintify_parfile_unrecognize():
    ...
    m = ModelBuilder()
    pint_dict, unknow = m._pintify_parfile(StringIO(wrong_par))
