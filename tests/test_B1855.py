...
class TestB1855(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(self):
        self.parfileB1855 = 'B1855+09_NANOGrav_dfg+12_TAI.par'
    ...
    def test_J1855_binary_delay(self):
        ...
        assert np.all(np.abs(pint_binary_delay.value + self.ltbindelay) < 1e-11), 'B1855 binary delay test failed.'

    def test_B1855(self):
        pint_resids_us = resids(self.toasB1855, self.modelB1855).time_resids.to(u.s)
        assert np.all(np.abs(pint_resids_us.value - self.ltres) < 4e-8), 'B1855 residuals test failed.'
