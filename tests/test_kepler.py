...
def test_kepler_3d_derivs():
    p = kepler.Kepler3DParameters(a=2, pb=3, eps1=0.2, eps2=0.1,
                                  i=np.pi/3, lan=np.pi/4, t0=-1)
    check_all_partials(
        flatten_namedtuple(kepler.kepler_3d,kepler.Kepler3DParameters),
                       list(p)+[4])

def test_kepler_3d_inverse():
    t = 23
    p = kepler.Kepler3DParameters(a=2, pb=3, eps1=0.2, eps2=0.1,
                                  i=np.pi/3, lan=np.pi/4, t0=t-1)
...
def test_kepler_two_body_derivs():
    p = kepler.KeplerTwoBodyParameters(
        a=2, pb=3, eps1=0.2, eps2=0.1,
        i=np.pi/3, lan=np.pi/4, q=0.2,
        x_cm=0, y_cm=0, z_cm=0,
        vx_cm=0, vy_cm=0, vz_cm=0,
        tasc=-1)
    check_all_partials(
        flatten_namedtuple(kepler.kepler_two_body,
                           kepler.KeplerTwoBodyParameters),
                       list(p)+[4])
